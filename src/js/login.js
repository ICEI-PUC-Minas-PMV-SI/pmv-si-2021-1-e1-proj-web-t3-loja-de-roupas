...
function initLoginApp () {
    ...
    var usuariosJSON = localStorage.getItem('dados');
    ...
    if (!usuariosJSON) {  // Se NÃO há dados no localStorage
        ...
        localStorage.setItem('dados', JSON.stringify (dadosIniciais));
    }
...
function loginUser(email, senha) {
    ...
    for (var i = 0; i < dados.length; i++) {
        ...
        if (email == usuario.email && senha == usuario.senha) {
            usuarioCorrente.id = usuario.nome;
...
function addUser (nome, usuario, senha, email) {
    
    // Cria um objeto de usuario para o novo usuario 
    let newId = generateUUID ();
    let usuarioDados = { "id": newId, "usuario": usuario, "senha": senha, "nome": nome, "email": email };
    
    // Inclui o novo usuario no banco de dados baseado em JSON
    dados.push(usuarioDados);
